...
try:
    ...
    print(f"ERROR: Failed to load models! Details: {e}")
    sys.exit()
...
def download_chess_pieces():
    ...
    if missing_files:
        print("Missing assets detected. Downloading chess pieces from Chess.com servers...")
        ...
        for fen_char, url in FEN_TO_URL.items():
            ...
            if not os.path.exists(filepath):
                try:
                    ...
                    print(f"Warning: Failed to download piece {fen_char}. ({e})")
...
# ==========================================
# MATRIX, RULE FILTER AND PYGAME INTERFACE
# ==========================================
...
def apply_chess_rules(board_matrix):
    """
    Automatically removes AI hallucinations
    that violate chess rules.
    """
    ...
    for r in range(8):
        for c in range(8):
            ...
            # RULE:
            # Pawns cannot exist on the 1st or 8th rank

            if piece.lower() == 'p' and (r == 0 or r == 7):

                print(
                    f"Referee Filter: AI detected an invalid pawn "
                    f"on row index {r}. Piece removed automatically!"
                )
...
def edit_board_pygame(board_matrix):
    ...
    pygame.display.set_caption(
        "AI Error Correction Interface (Drag & Drop)"
    )
    ...
    while running:
                ...
                if event.button == 1:
                    ...
                    if btn_rect.collidepoint(mouse_x, mouse_y):
                        ...
                        if "K" in fen_attempt and "k" in fen_attempt:
                            ...
                            error_msg = (
                                "ERROR: Both Kings (K/k) "
                                "must be present on the board!"
                            )

                    else:

                        c = (mouse_x - OFS_X) // SQ_SIZE
                        r = (mouse_y - OFS_Y) // SQ_SIZE
        ...
        screen.blit(
            btn_text,
            (btn_rect.x + 15, btn_rect.y + 12)
        )

        if error_msg:

            err_text = msg_font.render(
                error_msg,
                True,
                (255, 50, 50)
            )
...
def run_pipeline(warped_img):
    ...
    print("1. Local YOLO matrix generated.")

    board_matrix = generate_matrix_from_yolo(results, warped_img)

    print("2. Applying Referee Filter...")

    board_matrix = apply_chess_rules(board_matrix)

    print("3. Opening Pygame interface...")

    validated_fen = edit_board_pygame(board_matrix)

    if not validated_fen:

        print("Interface closed, analysis cancelled.\n")

        return

    print(f"4. Verified FEN: {validated_fen}")

    print("5. Stockfish is calculating the best move...")

    best_move = analyze_with_stockfish(validated_fen)

    print(f"\n=================================")
    print(f"♟️ RECOMMENDED BEST MOVE: {str(best_move).upper()}")
    print(f"=================================\n")
...
def main():
    ...
    print(
        "Camera initialized! Shortcuts:\n"
        "[A] Automatic Board Detection\n"
        "[R] Reset Points\n"
        "[C] Start Analysis (Drag & Drop Interface)\n"
        "[Q] Quit\n"
    )
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            if detected_corners is not None:
                ...
                print("Success! Board corners detected automatically.")

            else:

                print("ERROR: Board could not be detected clearly.")
            ...
            if len(points) == 4:
                ...
                print("Exactly 4 corners are required for analysis!")
